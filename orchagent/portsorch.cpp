...
PortsOrch::PortsOrch(DBConnector *db, DBConnector *stateDb, vector<table_name_with_pri_t> &tableNames, DBConnector *chassisAppDb) :
        ...
        port_phy_serdes_attr_manager(PORT_PHY_SERDES_ATTR_FLEX_COUNTER_GROUP, StatsMode::READ, PORT_STAT_FLEX_COUNTER_POLLING_INTERVAL_MS, false),
...
void PortsOrch::registerPort(Port &p)
{
    ...
    if (flex_counters_orch->getPortSerdesAttrCountersState())
    {
        if (!m_supported_phy_serdes_attrs.empty() &&
            ...
            supportsPortSerdesAttr(port_serdes_id, p.m_alias.c_str()))
        {
...
bool PortsOrch::supportsPortSerdesAttr(sai_object_id_t port_serdes_id, const char* port_name)
{
    sai_status_t status;
    sai_attribute_t test_attr;
    ...
    if (status != SAI_STATUS_BUFFER_OVERFLOW)
    {
        SWSS_LOG_ERROR("PORT_SERDES_ATTR: Port %s does not support SAI_PORT_SERDES_ATTR_RX_VGA attribute (status=%d)",
            port_name, status);
        // return false //TODO: production must return false.
    }
    ...
    if (status != SAI_STATUS_BUFFER_OVERFLOW)
    {
        SWSS_LOG_ERROR("PORT_SERDES_ATTR: Port %s does not support SAI_PORT_SERDES_ATTR_TX_FIR_TAPS_LIST attribute (status=%d)",
            port_name, status);
        //return false; //TODO: production must return false.
    }
...
void PortsOrch::generatePortSerdesAttrCounterMap()
{
    ...
    if (m_supported_phy_serdes_attrs.empty())
    {
        SWSS_LOG_WARN("PORT_SERDES_ATTR: No PORT SERDES attributes supported on this platform");
    ...
    for (const auto& it: m_portList)
    {
        if (it.second.m_type == Port::Type::PHY)
        {
            ...
            if (port_serdes_id == SAI_NULL_OBJECT_ID)
            {
                SWSS_LOG_ERROR("PORT_SERDES_ATTR: Port %s has no serdes object", port_name);
                continue;
            }

            if (supportsPortSerdesAttr(port_serdes_id, port_name))
            {
                SWSS_LOG_DEBUG("PORT_SERDES_ATTR: Setting counter ID list for port %s", port_name);
...
void PortsOrch::clearPortSerdesAttrCounterMap()
{
    ...
    for (const auto& it: m_portList)
    {
        ...
        if (port_serdes_id == SAI_NULL_OBJECT_ID)
        {
            SWSS_LOG_ERROR("PORT_SERDES_ATTR: Port %s has no serdes object", it.second.m_alias.c_str());
            continue;
        }
...
const std::vector<sai_port_serdes_attr_t>& PortsOrch::getPortSerdesAttrIds() const
{
    ...
    if (port_attr.value.oid != SAI_NULL_OBJECT_ID)
    {
        ...
        else
        {
            ...
            if (getPort(port_id, p) && p.m_type == Port::Type::PHY &&
                flex_counters_orch->getPortSerdesAttrCountersState())
            {
